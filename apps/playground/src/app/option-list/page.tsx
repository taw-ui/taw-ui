...
function InteractiveOptionList({
  ...
  label: string
  part: Parameters<typeof OptionList>[0]["part"]
}) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold tracking-tight">OptionList</h1>
        <p className="mt-1 text-sm text-zinc-400">
          Interactive single or multi-select option list with confirmation and receipt state.
        </p>
      </div>

      <div className="grid gap-8 lg:grid-cols-2">
        <Section label="Loading">
          <OptionList part={mocks.optionList.loading} />
        </Section>

        <Section label="Error">
          <OptionList part={mocks.optionList.error} />
        </Section>

        <InteractiveOptionList
          label="Single Select"
          part={mocks.optionList.single}
        />

        <InteractiveOptionList
          label="Multi Select"
          part={mocks.optionList.multi}
        />
      </div>
    </div>
  )
